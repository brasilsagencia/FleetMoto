...
export const RelatorioPdfModal: React.FC<RelatorioPdfModalProps> = ({
  ...
  const handlePrint = () => {
    printElementById('area-impressao-relatorio-pdf', {
      title: `Relatorio_${modeloAtivo.id}_${Date.now()}`
    });
  };
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white rounded-3xl max-w-4xl w-full border border-slate-200 shadow-2xl overflow-hidden my-6 flex flex-col max-h-[90vh]"
      >
        {/* Top Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-white">
                Exportar & Imprimir Relatório Oficial
              </h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5 lg:border-r lg:border-slate-200 lg:pr-6">
            <div className="space-y-2">
              ...
              <div className="space-y-3 pt-1 text-xs">
                <div>
                  <label className="text-slate-600 block mb-1">Orientação da Página:</label>
                ...
                <div>
                  <label className="text-slate-600 block mb-1">Nível de Detalhamento:</label>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="rounded border-slate-300 text-[#E05328] focus:ring-[#E05328]"
                    />
                    <span className="text-slate-800 font-medium">Incluir Campo de Assinaturas TSE</span>
                  </label>
                </div>

                <div className="pt-2">
                  <label className="text-slate-600 block mb-1">Notas / Observações no Rodapé:</label>
                  <textarea
                    value={observacoesPersonalizadas}
                    onChange={(e) => setObservacoesPersonalizadas(e.target.value)}
                    placeholder="Ex: Documento para prestação de contas no comitê financeiro..."
                    rows={2}
            ...
            <div className="pt-4 border-t border-slate-200 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="p-2.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                >
                ...
                <button
                  type="button"
                  onClick={onExportCsv}
                  className="p-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300 text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                >
              ...
              <div className="pt-2 flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 px-3 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-xs transition-all"
                >
          ...
          <div className="lg:col-span-2 bg-slate-100 p-4 rounded-2xl border border-slate-200 flex flex-col justify-between overflow-y-auto max-h-[60vh]">
            <div id="area-impressao-relatorio-pdf" className="bg-white p-6 rounded-xl border border-slate-300 shadow-sm space-y-4 text-slate-900 font-sans printable-area">
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                    <tr>
                      ...
                      <th className="p-2">Cliente / Descrição</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {itens.slice(0, 5).map((it, idx) => (
                      <tr key={idx}>
                        <td className="p-2 text-slate-600">{it.dataHoraFormatada || formatDate(it.dataHora)}</td>
                        <td className="p-2 font-mono font-bold text-slate-900">{it.numeroPedido || it.id.slice(0, 8)}</td>
                        <td className="p-2 text-slate-800 truncate max-w-[140px]">{it.clienteNome || it.materialNome || '-'}</td>
                        <td className="p-2 text-right font-bold text-slate-900">{it.quantidade ? formatNumber(it.quantidade) : '-'}</td>
                        <td className="p-2">
              ...
              {totalRegistros > 5 && (
                <p className="text-[11px] text-slate-500 text-center italic">
                  ... e mais {totalRegistros - 5} registros incluídos no documento completo.
                </p>
              )}
              ...
              {observacoesPersonalizadas && (
                <div className="bg-amber-50 p-2.5 rounded-lg border border-amber-200 text-xs text-amber-900">
                  <strong>Observações Adicionais:</strong> {observacoesPersonalizadas}
                </div>
              )}
        ...
        <div className="bg-slate-50 p-4 border-t border-slate-200 flex items-center justify-between">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl text-xs font-bold text-slate-700 bg-white border border-slate-300 hover:bg-slate-100 transition-all"
          >
            Cancelar
          </button>
          ...
          <button
            onClick={handlePrint}
            className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 shadow-md flex items-center gap-2 transition-all"
          >
